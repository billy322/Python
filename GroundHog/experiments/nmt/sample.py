...
class BeamSearch(object):
    ...
    def compile(self):
        # initialise values:
        # enc_dec = RNNEncoderDecoder object in encdec class
    ...
    def search(self, seq, n_samples, ignore_unk=False, minlen=1):
        '''Core of Beam_Search
        seq = input sentence stored in dict_index (source lang.)
        n_samples = no. of training sentences'''
        ...
        trans = [[]]
        costs = [0.0]
        ''' why 3*len(seq): read when need
            http://polarlion.github.io/nmt/2016/06/06/ground-show-alignment.html'''
        for k in range(3 * len(seq)):
            ...
            #self.comp_next_probs = output of NN (or decoder)
...
def sample(lm_model, seq, n_samples,
    ...
    # this function actally did a litte adjust on the result on class beamSearch
    if beam_search:
        sentences = []
...
def main():
    ...
    beam_search = None
    if args.beam_search:
        beam_search = BeamSearch(enc_dec) # this is the main things!!
        beam_search.compile()
    else:
    ...
    if args.source and args.trans:
        ...
        for i, line in enumerate(fsrc):
            seqin = line.strip()
            # parse_input = convert input to index in dict
